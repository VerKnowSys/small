defmodule Small do
  ...
  def process_event _event, file_path do
    if (File.exists? file_path) do
      Lager.debug "Handling event: #{inspect _event} for path #{file_path}"
      ...
      Lager.debug "#{inspect record}"
      QueueAgent.put record
      Sftp.add
    else
      Lager.debug "File doesn't exists: #{file_path} after event #{inspect _event}. Skipped process_event"
    end
  end
