defmodule SmallApplication do
  ...
  @spec launch_periodic_dump() :: {:error, any()} | {:ok, :timer.tref()}
  ...
  def main(param) do
    ...
    case SyncSupervisor.start_link() do
      {:ok, pid} ->
        ...
        if config()[:open_history_on_start] do
          ...
          Logger.info(
            "Automatically opening http dashboard: http://localhost:#{webapi_port()} in default browser."
          )
        ...
        case launch_periodic_dump() do
          {:ok, _pd_pid} ->
            Logger.debug(
              "Periodic dump spawned in background (triggered each #{dump_interval() / 360} hours)"
            )
    ...
    if param == "stay" do
      Logger.warning(" Eternal watch skipped on demand.")
    else
