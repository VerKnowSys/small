defmodule WebApi.Handler do
  ...
  @spec init({any(), :http}, any(), []) :: {:ok, any(), :undefined}
  @spec init({any(), :http}, any(), []) :: {:ok, any(), :undefined}
  def init({_any, :http}, req, []) do
    {:ok, req, :undefined}
  end

  @spec extract_links(timestamp :: String.t, links :: String.t, file :: String.t) ::
          String.t
  ...
  @spec outer_route(Enum.t, :cowboy_req.req(), any()) :: {:ok, :cowboy_req.req(), any()}
  def outer_route(collection, req, state) do
    ...
    {:ok, req} =
      :cowboy_req.reply(
        ...
        "<html>" <>
          head() <>
          "<body><pre class=\"count\"><span>small</span> history of: #{size}</pre></div><div>" <>
          (collection
           |> Enum.map(fn %Database.History{content: links, timestamp: ts, file: file, uuid: uuid} ->
             links_list = links |> String.split(" ")

             "<article id=\"#{uuid}\" class=\"text-center\">" <>
               extract_links(ts, links_list, file) <> "</article>"
           end)
           |> Enum.join(" ")) <>
          "</div><footer><pre class=\"count\">Sync eM ALL - version: #{version()} - © 2015-2017 - Daniel (@dmilith) Dettlaff</pre></footer></body></html>",
        req
      )
  ...
  @spec route(String.t, any(), :cowboy_req.req(), any()) :: {:ok, :cowboy_req.req(), any()}
  ...
  @spec handle(any(), any()) :: {:ok, any(), any()}
  def handle(req, state) do
    case req do
      {:http_req, _, :ranch_tcp, :keepalive, _pid, "GET", :"HTTP/1.1", {{_, _, _, _}, _}, _, _, _,
       path, _, args, _, [], _, [{"connection", ["keep-alive"]}], _, [], _, "", _, _, _, [], "",
       _} ->
