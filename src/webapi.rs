...
impl WebApi {
    pub fn new(config: Arc<AppConfig>, database: Arc<Database>) -> Self {
        WebApi { config, database }
    }

    pub async fn start(self: Arc<Self>) -> Result<()> {
        let port = self.config.webapi_port;
        log::info!("Launching Small WebApi on http://127.0.0.1:{}", port);
    ...
    async fn handle_request(
        ...
        log::debug!("Loading history of {} elements (default)", limit);
    ...
    async fn handle_count_request(&self, count: usize) -> Result<impl warp::Reply, Infallible> {
        log::info!("Loading history of {} elements.", count);
    ...
    fn render_history(&self, history: &[crate::database::History]) -> String {
